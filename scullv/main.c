 ...
 */
/* static void scullv_do_deferred_op(void *p) tpb */
static void scullv_do_deferred_op(struct work_struct *work) /* tpb */
{
		...
		size_t count, loff_t pos)
{
	...
	stuff->iocb = iocb;
	stuff->result = result;
/*	INIT_WORK(&stuff->work, scullv_do_deferred_op, stuff); tpb */
	INIT_DELAYED_WORK(&stuff->work, scullv_do_deferred_op); /* tpb */
...
static ssize_t scullv_aio_read(struct kiocb *iocb,
		...
		loff_t pos)
{
/*	return scullv_defer_op(0, iocb, buf, count, pos); tpb */
...
static ssize_t scullv_aio_write(struct kiocb *iocb,
		...
		loff_t pos)
{
/*	return scullv_defer_op(1, iocb, (char __user *) buf, count, pos); tpb **/
        return 0; /* FIX ME - aio treatment in this example is broken. tpb */
}
